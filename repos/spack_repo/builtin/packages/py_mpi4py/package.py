...
class PyMpi4py(PythonPackage):
    ...
    def create_mpi_config_file(self, cfg_fn):
        ...
        with open(cfg_fn, "w") as cfg:
            cfg.write("[mpi]\n")
            cfg.write(f"mpi_dir              = {mpi_spec.prefix}\n")
            cfg.write(f"mpicc                = {mpi_spec.mpicc}\n")
            cfg.write(f"mpicxx               = {mpi_spec.mpicxx}\n")
            cfg.write("\n")
            cfg.write("## define_macros        =\n")
            cfg.write("## undef_macros         =\n")
            cfg.write(f"include_dirs         = {include_dirs}\n")
            cfg.write("## libraries            = mpi\n")
            cfg.write(f"library_dirs         = {library_dirs}\n")
